...
Wails 桌面模式启动后，界面主要分为以下几个页面：

- `Dashboard`：查看当前连接状态、节点摘要和最近活动
- `Connection`：配置网关连接、测试连通性、发起连接或断开连接
- `Capabilities`：开启或关闭节点能力
- `Operations`：执行设备操作
- `Logs`：查看 GUI 运行日志
- `About`：查看设备 ID、公钥、运行平台和数据目录
...
1. 打开 `Connection` 页面
2. 填写 `网关地址`、`端口`、`网关令牌`
3. 根据网关环境设置 `TLS` 和 `发现方式`
4. 点击 `保存配置` 保存配置
5. 点击 `测试连接` 测试当前网关是否可连通
6. 点击 `连接` 建立连接
7. 回到 `Dashboard` 查看连接状态与最近活动
8. 在 `Capabilities` 页面按需启用或关闭能力

### `Connection` 页面字段说明

- `网关地址`：网关地址，可填写主机名、`host:port` 或完整 URL
- `端口`：网关端口；当 `网关地址` 未显式带端口时会用该值补齐
- `发现方式`：
  - `自动（mDNS）`：使用局域网自动发现
  - `手动`：使用手动配置的网关地址
- `网关令牌`：连接网关时使用的鉴权 Token
- `TLS`：开启后优先使用安全 WebSocket 连接

页面按钮行为：

- `保存配置`：保存当前 GUI 配置到本地配置文件
- `测试连接`：按当前配置发起一次 WebSocket 连通性测试
- `连接`：连接当前配置的网关
- `断开连接`：断开当前会话

### `Capabilities` 页面说明

`Capabilities` 页面中的开关会直接更新本地配置，并在运行时重新应用能力状态。  
这意味着 GUI 中对能力开关的修改不仅影响当前会话，也会影响下次启动时的默认状态。
